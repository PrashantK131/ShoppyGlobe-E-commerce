        ...
        <div className="flex items-center gap-4 p-4 rounded-2xl transition-all" style={{ background: '#fff', border: '1px solid var(--color-pebble)' }}>
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-2">
                    <button onClick={() => dispatch(decrementQuantity(item.id))} disabled={item.quantity <= 1} className="w-7 h-7 rounded-full flex items-center justify-center font-bold text-sm transition-colors disabled:opacity-30"
                        style={{ background: 'var(--color-pebble)', color: 'var(--color-charcoal)' }} aria-label="Decrease quantity"
                    >
                        −
                    </button>
                    <span className="font-semibold text-sm w-5 text-center">{item.quantity}</span>
                    <button onClick={() => dispatch(incrementQuantity(item.id))} className="w-7 h-7 rounded-full flex items-center justify-center font-bold text-sm transition-colors"
                        style={{ background: 'var(--color-charcoal)', color: '#fff' }} aria-label="Increase quantity"
                    >
                        +
                    </button>
                </div>
