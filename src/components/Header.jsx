// Navigation bar with cart icon and search
        ...
        <header style={{ background: 'var(--color-charcoal)', fontFamily: 'var(--font-body)'}} className="sticky top-0 z-50 shadow-lg">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
                ...
                <div className="flex-1 max-w-md hidden sm:block">
                    <input type="text" placeholder="Search products…" value={searchValue} onChange={e => onSearchChange(e.target.value)} style={{ background: 'rgba(255,255,255,0.1)', color: 'var(--color-cream)', borderColor: 'rgba(255,255,255,0.2)'}}
                ...
                <nav className="flex items-center gap-5">
                    <NavLink to="/" style={({ isActive }) => ({color: isActive ? 'var(--color-clay)' : 'var(--color-cream)',fontWeight: isActive ? '600' : '400'})} className="text-sm hidden sm:block hover:opacity-80 transition-opacity">
                        Home
                    </NavLink>
                    ...
                    <Link to="/cart" className="relative flex items-center gap-1.5 group"  style={{ color: 'var(--color-cream)' }}>
                        ...
                        {cartCount > 0 && (
                        <span key={cartCount} style={{ background: 'var(--color-clay)' }} className="badge-pop absolute -top-2 -right-2 text-white text-xs font-semibold min-w-[18px] h-[18px] rounded-full flex items-center justify-center px-1">
                            {cartCount}
                        </span>
                        )}
                    </Link>
